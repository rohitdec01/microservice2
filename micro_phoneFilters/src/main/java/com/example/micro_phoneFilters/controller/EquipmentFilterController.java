...
public class EquipmentFilterController {
	...
	public ResponseEntity<List<FilterCategory>> getFilters() {
		...
		List<EquipmentFilter> ef = equipmentFilters.get(0).getEquipmentFilters();
		//sequipmentFilters.get(1).setEquipmentFilters(null);
		//String aaa = (new ResponseEntity<List<FilterCategory>>(equipmentFilters, HttpStatus.OK)).toString();
		return new ResponseEntity<List<FilterCategory>>(equipmentFilters, HttpStatus.OK);
	}
	
	/*@RequestMapping(value = "/filters2", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<EquipmentFilter>> getFilters2() {
		System.out.println("inside Filter controller");
		List<EquipmentFilter> equipmentFilters = equipmentFilterService.getEquipmentFilters2();
		if (equipmentFilters == null) {
			return new ResponseEntity<List<EquipmentFilter>>(HttpStatus.NO_CONTENT);
		}
		return new ResponseEntity<List<EquipmentFilter>>(equipmentFilters, HttpStatus.OK);
	}*/
